...
random.seed(1234)
# target_dataset = 'NYC' # ['NYC', 'TKY']
target_dataset = 'TKY' # ['NYC', 'TKY']
source_pth = f'./raw/dataset_TSMC2014_{target_dataset}.txt'

dist_pth = f'./processed/{target_dataset.lower()}'
col_names = ['uid', 'poi', 'catid', 'catname','lat', 'lon', 'offset', 'time']
review_pth = join(dist_pth, 'all_data.pkl')

if not os.path.exists(review_pth) or True:
    print(f'Load from {source_pth}\nData preprocessing...')
    review_df = pd.read_csv(source_pth, sep='\t', header=None, names=col_names, encoding='unicode_escape')
    ...
    review_df['date'] = pd.to_datetime(review_df['time']).dt.date
